...
public class FengGameManagerMKII : Photon.MonoBehaviour
{
    ...
    {
        InRoomChat.Message(GameObject.Find("MainCamera").GetComponent<IN_GAME_MAIN_CAMERA>().main_object.transform.position.x.ToString());
        InRoomChat.Message(GameObject.Find("MainCamera").GetComponent<IN_GAME_MAIN_CAMERA>().main_object.transform.position.y.ToString());
        InRoomChat.Message(GameObject.Find("MainCamera").GetComponent<IN_GAME_MAIN_CAMERA>().main_object.transform.position.z.ToString());
    }
                        ...
                        else
                        {
                            ...
                            else if (((int) settings[0x40]) == 10)
                            {
                                ...
                                else if (((int) settings[230]) == 2)
                                {
                                    ...
                                    GUI.Label(new Rect(num7 + 100f, num8 + 274f, 160f, 22f), "Hide & Seek Mod:", "Label");
                                    bool has = false;
            ...
            else
            {
                ...
                if (PhotonNetwork.isMasterClient)
                {
                    ...
                    else if ((info.type != GAMEMODE.TROST) && ((info.type == GAMEMODE.PVP_CAPTURE) && (LevelInfo.getInfo(FengGameManagerMKII.level).mapName == "OutSide")))
                    {
                        ...
                        {
                            this.spawnTitanRaw(objArray3[i].transform.position, objArray3[i].transform.rotation).GetComponent<TITAN>().setAbnormalType(AbnormalType.TYPE_CRAWLER, true);
                        }
                    }
                }
                if (!info.supply)
    ...
    public void restartGame(bool masterclientSwitched)
    {
        if (!this.gameTimesUp)
        {
            ...
            base.photonView.RPC("settingRPC", PhotonTargets.Others, new object[] {  });
            base.photonView.RPC("RPCLoadLevel", PhotonTargets.All, new object[0]);
            this.setGameSettings(hash);
            if (masterclientSwitched)
            {
                this.sendChatContentInfo("<color=#A8FF24>MasterClient has switched to </color>" + RCextensions.returnStringFromObject(PhotonNetwork.player.customProperties[PhotonPlayerProperty.name]).hexColor());
            }
        }
    }
    ...
    {
        RCPausing = !RCPausing;
        photonView.RPC("pauseRPC", PhotonTargets.All, new object[] { RCPausing });
        photonView.RPC("Chat", PhotonTargets.All, new object[] { "<color=#FFCC00>MasterClient has " + (RCPausing ? "paused" : "unpaused") + " the game.</color>", "" });
    }
    ...
    private void Update()
    {
        if (IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.SINGLE && GameObject.Find("LabelNetworkStatus"))
        {
            ...
            if (PhotonNetwork.connected)
            {
                UILabel expr_5A = GameObject.Find("LabelNetworkStatus").GetComponent<UILabel>();
    ...
    {
        yield return new WaitForSeconds(time);
        this.SpawnPlayerAt(this.myLastHero, pos);
    }
